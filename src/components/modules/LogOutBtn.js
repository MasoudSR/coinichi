function LogOutBtn({ toggle, setToggle, logout }) {
	return (
		<div className="relative">
			<div
				className={`mt-4 flex transition-all pb-4 duration-500 b-4 ${toggle ? "gap-3" : "gap-0"} select-none`}>
				<div
				...
				`}
					...
					<span
						className={`transition-all w-[50%] duration-500 text-right origin-left ${
							toggle ? "scale-0" : "scale-100"
						}`}>
						Log
					</span>
